...
- [x] Package only the official `ultradex_sdk` namespace, require Python 3.11+, and remove deprecated GitLab/invalid Python CLI metadata.
...
- [x] Forward idempotency, correlation, and actor headers to `CommandRequest`; keep actor correlation distinct from explicit delegation authority.
- [x] Prove repeated idempotent submission returns stable IDs without another enqueue.
...
- [x] Return operation detail/list and chronological nested events.
- [x] Expose source event identity/position, projection timestamp, bounded lag, and contract-valid freshness status.
