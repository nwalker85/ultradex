...
### Forgejo restoration (2026-07-29)

- Canonical repository: private Forgejo `nate/ultradex`; GitHub is a passive mirror
  and is not the review or CI gate.
- Canonical base: Forgejo `main` at
  `5695a04cfa67d976d73a9256f6c8518edb68d3c1`.
- Restoration branch: `feat/jobsearch-governed-commands-forgejo`, rebased from the
  original JS-U03 tip without rewriting or deleting
  `feat/jobsearch-governed-commands`.
- This restoration does not claim the code is merged, deployed, or live.
...
- [x] Inspect:
  `git diff --check`,
  `git status --short`,
  `git log --oneline forgejo/main..HEAD`,
  and `git diff --stat forgejo/main...HEAD`.
...
- [x] Update the execution manifest with JS-U03 PR evidence and the honest JS-U04
  dependency state.
- [x] Push `feat/jobsearch-governed-commands-forgejo` to Forgejo and open canonical
  [Forgejo PR 3](http://hrafngud.taild7ad1f.ts.net:3300/nate/ultradex/pulls/3)
  against `main`.
